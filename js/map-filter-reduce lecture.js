...
let totalVal = prices.reduce((total, itemPrice, index, array) =>{
    // console.log(`the index is ${index}, the total is ${total.toFixed(2)}, the itemPrice is ${itemPrice}`);

    return total + itemPrice;
});

// let averagePrice = prices.reduce((acc, itemPrice, index, array) =>{
//     acc += itemPrice;
//     if (index === array.length-1) {
...
const totalCost = prices.reduce(function (total, priceOfItem){
   return total + priceOfItem;
}, 158.76);

console.log(`your total is ${totalCost.toFixed(2)}`);

// const mileages = cars.reduce((acc, element, index, array) =>{
//     // if (index === array.length -1){
//     //     return acc.sort(function (a, b ,c){return b-a[0]});
//     // }
//     acc.push(element.mileage);
//     return acc;
// }, []);
